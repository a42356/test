$(document).ready(function() {
  "use strict";
	//$('.dm').next().appendTo($('.dm-product'));

  // $('.floor').each(function(){
  //   $(this).next().appendTo($(this).find('.slick-block'));
  // });

  //$('.product-block .commodity .slick-block table td').unwrap().unwrap().unwrap();
  //$('.product-block .commodity .slick-block thead').remove();

	
  //DM同步
  $('.dm-product .slick-block').slick({
    dots: false,
    infinite: false,
    fade: true,
    prevArrow:
      '<button type="button" data-role="none" class="slick-prev" aria-label="Previous" tabindex="0" role="button"><img src="images/left.png"></button>',
    nextArrow:
      '<button type="button" data-role="none" class="slick-next" aria-label="Next" tabindex="0" role="button"><img src="images/right.png"></button>',
    slidesPerRow: 2,
    rows: 2,
    responsive: [
      {
  ...
  $(window).on('resize orientationchange', function() {
    $('.product-block .commodity .slick-block').slick('resize');
  });
...
//  });
  ...
  $('.search-button').click(function() {
    ...
    $('.spacer').each(function() {
      ...
      if (
        dataIndex.search(inputValue.toLowerCase()) !== -1 &&
        inputValue !== ''
      ) {
...
//   });
  ...
  $(window).scroll(function() {
    ...
    if ($(window).width() > 975) {
      ...
      //DM同步
      if (
        scroll >= $('#makeup').position().top - 200 &&
        scroll < $('#fashions').position().top - 200
      ) {
        $('.floor-button').removeClass('active');
        $('.floor-makeup').addClass('active');
      }
//		  (
//        scroll >= $('#dm').position().top - 200 &&
//        scroll < $('.photo').position().top - 200
//      ) {
//        $('.floor-button').removeClass('active');
...
//      }
      //樓層2
        ...
        scroll >= $('#fashions').position().top - 200 &&
        scroll < $('#clothing').position().top - 200
      ) {
        $('.floor-button').removeClass('active');
      ...
      //樓層6
        ...
        scroll >= $('#kids2').position().top - 200 &&
        scroll < $('#homeneeds').position().top - 200
      ) {
        $('.floor-button').removeClass('active');
        $('.floor-kids2').addClass('active');
      }
      //樓層7
      else if (
        scroll >= $('#homeneeds').position().top - 200 &&
        scroll < $('#food').position().top - 200
      ) {
        $('.floor-button').removeClass('active');
    ...
    if ($(window).width() <= 975) {
      ...
      //樓層1
      if (
        docViewBottom > $('.floor1-image').position().top &&
        scroll < $('#makeup').position().top
      ) {
        ...
        $('.floor1-image')
          ...
          .next()
          .next()
          .css({ transform: 'translate(0%,-' + pxspeed * 0.1 + '%)' });
      }
      //樓層2
      else if (
        docViewBottom > $('.floor2-image').position().top &&
        scroll < $('#fashions').position().top
      ) {
      ...
      //樓層3
        ...
        docViewBottom > $('.floor3-image').position().top &&
        scroll < $('#clothing').position().top
      ) {
      ...
      //樓層4
        ...
        docViewBottom > $('.floor4-image').position().top &&
        scroll < $('#sport').position().top
      ) {
      ...
      //樓層5
        ...
        docViewBottom > $('.floor5-image').position().top &&
        scroll < $('#kids').position().top
      ) {
      ...
      //樓層6
        ...
        docViewBottom > $('.floor6-image').position().top &&
        scroll < $('#homeneeds').position().top
      ) {
      ...
      //樓層7
        ...
        docViewBottom > $('.floor7-image').position().top &&
        scroll < $('#food').position().top
      ) {
